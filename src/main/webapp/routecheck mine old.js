  ...
  db.collection("routes").get().then((snapshot) => {
    snapshot.docs.forEach(doc => {
      ...
      var contentString = '<div id="content">' +
        ...
        '<button type="button" id="addComm" >Add Comment</button> ';
...
/*
  To get all messages in roomA - given you have a collection of rooms with each room having its messages (SUBCOLECTION)
...
function addComment(doc) {
  const cmtContent = cmtForm['cmt-box'].value;

  var commentsRef = db.collection('routes').doc(doc).collection('comments');
